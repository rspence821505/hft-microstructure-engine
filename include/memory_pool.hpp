 ...
 * @brief Week 4.2: Performance Optimization - Cache-optimized memory pool allocator
 *
...
public:
    ...
    size_t memory_usage() const { return chunks_.size() * chunk_size_ + large_allocations_.size() * chunk_size_; }
...
private:
    ...
    static constexpr size_t PADDED_SIZE = ((sizeof(T) + CACHE_LINE_SIZE - 1) / CACHE_LINE_SIZE) * CACHE_LINE_SIZE;
...
public:
    ...
    size_t peak_allocated() const { return peak_allocated_.load(std::memory_order_relaxed); }

    double utilization() const {
        return 100.0 * allocated() / capacity_;
    }
};
 ...
 * Pre-allocates a fixed number of slots and provides O(1) get/release operations.
 *
 * @tparam T Object type
...
inline void print_arena_stats() {
    ...
    std::cout << "  Total allocated: " << g_thread_arena.total_allocated() << " bytes\n";
    std::cout << "  Allocation count: " << g_thread_arena.allocation_count() << "\n";
    std::cout << "  Chunk count: " << g_thread_arena.chunk_count() << "\n";
    std::cout << "  Memory usage: " << g_thread_arena.memory_usage() << " bytes\n";
