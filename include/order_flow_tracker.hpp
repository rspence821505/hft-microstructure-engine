#ifndef ORDER_FLOW_TRACKER_HPP
#define ORDER_FLOW_TRACKER_HPP
...
class OrderFlowTracker {
public:
    static constexpr size_t MAX_WINDOWS = 60;  // Keep 1 hour of history (60 x 1min windows)
...
public:
     ...
     */
    void set_max_windows(size_t max_windows) {
    ...
    void print_statistics() const {
        ...
        std::cout << "Window duration: " << window_duration_.count() << " seconds\n";
...
public:
    ...
    void record_fill(const EnhancedFill& fill) {
        ...
        if (it == trackers_.end()) {
            auto [new_it, _] = trackers_.emplace(
                fill.symbol, OrderFlowTracker(window_seconds_));
            it = new_it;
        }
