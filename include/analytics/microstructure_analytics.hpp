...
private:
  ...
  std::unordered_map<std::string, RollingStatistics<double, PRICE_HISTORY_SIZE>>
      price_history_;
...
public:
  ...
  FillCallback create_fill_callback() {
    return [this](const EnhancedFill &fill) { this->process_fill(fill); };
  }
   ...
   */
  double get_buy_ratio() const { return flow_tracker_.get_buy_ratio(); }

  /**
   * @brief Gets the aggregate flow tracker
   * @return Reference to the flow tracker
   */
  const OrderFlowTracker &get_flow_tracker() const { return flow_tracker_; }
   ...
   */
  const SimpleImpactModel &get_impact_model() const { return impact_model_; }

  /**
   * @brief Gets mutable impact model for calibration
   * @return Reference to the impact model
   */
  SimpleImpactModel &get_impact_model() { return impact_model_; }
   ...
   */
  void set_auto_calibrate(bool enable) { auto_calibrate_impact_ = enable; }
   ...
   * @return true if calibration was successful
   */
  bool calibrate_from_enhanced_fills(const std::vector<EnhancedFill> &fills,
                                     uint64_t adv) {
    calibrated_impact_model_ = ::calibrate_impact_model(fills, adv);
   ...
   */
  double estimate_calibrated_impact(uint64_t volume,
   ...
   */
  double estimate_permanent_impact(uint64_t volume,
                                   const std::string &symbol) const {
    uint64_t adv = get_symbol_adv(symbol);
    if (use_calibrated_model_) {
   ...
   */
  double estimate_temporary_impact(uint64_t volume,
                                   const std::string &symbol) const {
    uint64_t adv = get_symbol_adv(symbol);
    if (use_calibrated_model_) {
   ...
   */
  bool is_using_calibrated_model() const { return use_calibrated_model_; }

  /**
   * @brief Enables or disables use of calibrated model
   * @param use Whether to use the calibrated model
   */
  void set_use_calibrated_model(bool use) { use_calibrated_model_ = use; }
   ...
   */
  void clear_calibration_fills() { calibration_fills_.clear(); }
  ...
  void print_calibration_summary() const {
    ...
    std::cout << "Using calibrated model: "
              << (use_calibrated_model_ ? "Yes" : "No") << "\n";
   ...
   */
  const TradeMetrics &get_current_metrics() const { return current_metrics_; }
   ...
   * @return Fill count
   */
  uint64_t get_total_fills_processed() const { return total_fills_processed_; }

  /**
   * @brief Enables or disables per-symbol tracking
   * @param enable Whether to track per symbol
   */
  void set_per_symbol_tracking(bool enable) { track_per_symbol_ = enable; }
  ...
  void print_summary() const {
    ...
    std::cout << "  Total buy volume: " << flow_tracker_.get_total_buy_volume()
              << "\n";
    std::cout << "  Total sell volume: "
              << flow_tracker_.get_total_sell_volume() << "\n";
    ...
    if (track_per_symbol_) {
      ...
      for (const auto &symbol : symbol_flow_tracker_.get_symbols()) {
        ...
        if (tracker) {
          std::cout << "  " << symbol
                    << ": imbalance=" << tracker->compute_current_imbalance()
...
private:
  ...
  void maybe_record_impact_observation(const EnhancedFill &fill) {
    ...
    if (fill.base_fill.quantity < 100)
      return;
