 ...
 * @brief Performance Optimization - Comprehensive monitoring and benchmarking
 *
 ...
 * @brief Lock-free performance monitoring with latency histograms and
 * throughput tracking
 *
 * Uses atomic operations for thread-safe statistics collection with minimal
 * contention. Latency is bucketed into 100ns intervals for fast histogram
 * updates.
 *
 * Features:
...
public:
  ...
  static constexpr uint64_t MAX_TRACKED_NS =
      NUM_BUCKETS * BUCKET_SIZE_NS; // 10us max
...
private:
  ...
  alignas(64)
      std::array<std::atomic<uint64_t>, NUM_US_BUCKETS> us_latency_histogram_;
...
public:
  ...
  void record_event_latency(uint64_t latency_ns) {
    ...
    if (latency_ns < MAX_TRACKED_NS) {
      ...
      latency_histogram_[bucket].fetch_add(1, std::memory_order_relaxed);
               ...
               MAX_TRACKED_NS + NUM_US_BUCKETS * US_BUCKET_SIZE_NS) {
  ...
  void print_csv() const {
    std::cout << name_ << "," << events_processed_.load() << "," << std::fixed
              << std::setprecision(1) << mean_latency() << "," << get_p50_ns()
              << "," << get_p95_ns() << "," << get_p99_ns() << ","
              << get_p999_ns() << "," << get_min_latency_ns() << ","
              << get_max_latency_ns() << "," << std::setprecision(0)
              << throughput() << "\n";
  }
  ...
  static void print_csv_header() {
    std::cout << "name,events,mean_ns,p50_ns,p95_ns,p99_ns,p999_ns,min_ns,max_"
                 "ns,throughput\n";
  }
  ...
  bool check_targets(uint64_t target_p50_ns, uint64_t target_p99_ns,
    ...
    std::cout << "  " << name_ << ":\n";
    std::cout << "    p50 <= " << target_p50_ns
              << "ns: " << (p50_ok ? "PASS" : "FAIL")
              << " (actual: " << get_p50_ns() << "ns)\n";
    std::cout << "    p99 <= " << target_p99_ns
              << "ns: " << (p99_ok ? "PASS" : "FAIL")
              << " (actual: " << get_p99_ns() << "ns)\n";
    std::cout << "    throughput >= " << std::fixed << std::setprecision(0)
              << target_throughput
              << "/sec: " << (throughput_ok ? "PASS" : "FAIL")
...
public:
  ...
  PerformanceMonitor &order_book_update() {
    return monitors_[ORDER_BOOK_UPDATE];
  }
  PerformanceMonitor &analytics_compute() {
    return monitors_[ANALYTICS_COMPUTE];
  }
  PerformanceMonitor &queue_handoff() { return monitors_[QUEUE_HANDOFF]; }
  PerformanceMonitor &impact_calibration() {
    return monitors_[IMPACT_CALIBRATION];
  }
  PerformanceMonitor &execution_strategy() {
    return monitors_[EXECUTION_STRATEGY];
  }
  PerformanceMonitor &end_to_end() { return monitors_[END_TO_END]; }
  PerformanceMonitor &custom() { return monitors_[CUSTOM]; }

  const PerformanceMonitor &csv_parsing() const {
    return monitors_[CSV_PARSING];
  }
  const PerformanceMonitor &order_book_update() const {
    return monitors_[ORDER_BOOK_UPDATE];
  }
  const PerformanceMonitor &analytics_compute() const {
    return monitors_[ANALYTICS_COMPUTE];
  }
  const PerformanceMonitor &queue_handoff() const {
    return monitors_[QUEUE_HANDOFF];
  }
  const PerformanceMonitor &impact_calibration() const {
    return monitors_[IMPACT_CALIBRATION];
  }
  const PerformanceMonitor &execution_strategy() const {
    return monitors_[EXECUTION_STRATEGY];
  }
  ...
  const PerformanceMonitor &get(ComponentIndex idx) const {
  ...
  /**
   * @brief Verifies performance targets
   *
   ...
   * - End-to-end latency: <10us from market data to analytics result
   */
  void verify_performance_targets() const {
    std::cout << "\n========================================\n";
    ...
    if (monitors_[CSV_PARSING].events_processed() > 0) {
      ...
      bool ok =
          monitors_[CSV_PARSING].check_targets(5000,  // p50 < 5us (reasonable)
                                               10000, // p99 < 10us
    ...
    if (monitors_[ORDER_BOOK_UPDATE].events_processed() > 0) {
      ...
      bool ok =
          monitors_[ORDER_BOOK_UPDATE].check_targets(1000,   // p50 < 1us
                                                     2000,   // p99 < 2us
                                                     1000000 // >1M ops/sec
          );
      all_pass &= ok;
    ...
    if (monitors_[ANALYTICS_COMPUTE].events_processed() > 0) {
      ...
      bool ok =
          monitors_[ANALYTICS_COMPUTE].check_targets(500,   // p50 < 500ns
                                                     1000,  // p99 < 1us
                                                     500000 // >500K ops/sec
          );
      all_pass &= ok;
    ...
    if (monitors_[QUEUE_HANDOFF].events_processed() > 0) {
      ...
      bool ok = monitors_[QUEUE_HANDOFF].check_targets(100,     // p50 < 100ns
                                                       500,     // p99 < 500ns
    ...
    if (monitors_[END_TO_END].events_processed() > 0) {
      ...
      bool ok = monitors_[END_TO_END].check_targets(10000, // p50 < 10us
                                                    50000, // p99 < 50us
  ...
  void print_summary_table() const {
    std::cout << "\n";
    std::cout << std::left << std::setw(20) << "Component" << std::setw(12)
              << "Events" << std::setw(12) << "p50 (ns)" << std::setw(12)
              << "p99 (ns)" << std::setw(15) << "Throughput"
              << "\n";
    std::cout << std::string(71, '-') << "\n";

    for (const auto &m : monitors_) {
      if (m.events_processed() > 0) {
        std::cout << std::left << std::setw(20) << m.get_name() << std::setw(12)
                  << m.events_processed() << std::setw(12) << m.get_p50_ns()
                  << std::setw(12) << m.get_p99_ns() << std::setw(15)
                  << std::fixed << std::setprecision(0) << m.throughput()
                  << "/s"
                  << "\n";
      }
    }
