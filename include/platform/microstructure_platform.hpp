 ...
 * @brief Connect All Components - Main Integration Platform
 *
 * This is the central integration point for the Microstructure Analytics
 ...
 * Architecture:
 *   Historical Layer:
 *     CSV Parser --> Event Timeline --> Backtesting Engine --> Impact Model
 * Calibration
 *
 *   Real-Time Layer:
 *     TCP Feed --> Ring Buffer --> Lock-Free Queue --> Protocol Parser -->
 * Analytics Engine
 *
 *   Execution Layer:
...
struct ExecutionComparison {
  ...
  void print() const {
    ...
    std::cout << std::left << std::setw(20) << "Algorithm" << std::setw(15)
              << "Avg Price" << std::setw(18) << "Shortfall (bps)"
              << std::setw(12) << "Fill Rate" << std::setw(10) << "Trades"
              << "\n";
    std::cout << std::string(75, '-') << "\n";

    for (const auto &r : results) {
      std::cout << std::left << std::setw(20) << r.name << std::setw(15)
                << std::fixed << std::setprecision(2) << r.avg_price
                << std::setw(18) << std::setprecision(2)
                << r.implementation_shortfall_bps << std::setw(12)
                << std::setprecision(1) << (r.fill_rate * 100) << "%"
...
public:
   ...
   */
  explicit MicrostructureAnalyticsPlatform(
      const PlatformConfig &config = PlatformConfig())
      : config_(config) {}

  ~MicrostructureAnalyticsPlatform() { stop(); }
  ...
  void initialize() {
    ...
    analytics_ =
        std::make_unique<MicrostructureAnalytics>(config_.flow_window_seconds);
    ...
    feed_aggregator_->set_tick_callback(
        [this](const AggregatedTick &tick) { on_aggregated_tick(tick); });
  ...
  void load_historical_data(const std::string &filename) {
    ...
    if (config_.verbose) {
      std::cout << "[Platform] Loaded " << backtester_->timeline_size()
                << " events\n";
    }
  }
   ...
   */
  void add_feed(const std::string &name, const std::string &host,
                uint16_t port) {
    ensure_initialized();
  ...
  AnalyticsSnapshot get_snapshot() const {
    ...
    if (analytics_) {
      ...
      snapshot.flow.buy_volume =
          analytics_->get_flow_tracker().get_total_buy_volume();
      snapshot.flow.sell_volume =
          analytics_->get_flow_tracker().get_total_sell_volume();
    }

    if (has_calibrated_model_) {
      snapshot.estimated_impact_100k =
          calibrated_impact_model_.estimate_total_impact(100000,
                                                         config_.assumed_adv);
      snapshot.estimated_impact_1m =
          calibrated_impact_model_.estimate_total_impact(1000000,
                                                         config_.assumed_adv);
    }
  ...
  void print_analytics_snapshot() const {
    ...
    if (has_calibrated_model_) {
      ...
      std::cout << "  100K shares: " << snapshot.estimated_impact_100k
                << " bps\n";
      std::cout << "  1M shares: " << snapshot.estimated_impact_1m << " bps\n";
    }

    std::cout << "Performance:\n";
    std::cout << "  Events processed: " << snapshot.events_processed
              << std::endl;
   ...
   */
  ExecutionComparison
  test_execution_strategies(const std::string &replay_file) {
    ensure_initialized();
    ...
    auto twap_result = backtester_->test_execution_strategy(&twap_strategy,
                                                            symbol, target_qty);
    ...
    twap_algo.implementation_shortfall_bps =
    ...
    for (int minutes : {15, 60, 120}) {
      ...
      auto result = backtester_->test_execution_strategy(&twap_variant, symbol,
                                                         target_qty);
...
private:
  ...
  void on_aggregated_tick(const AggregatedTick &tick) {
    ...
    int order_id =
        static_cast<int>(performance_monitor_->events_processed() + 1);
